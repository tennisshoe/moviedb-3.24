...
{ 
  ...
  titleIndex = (struct titleKeyOffset *) calloc ( titleCount * 2, sizeof ( struct titleKeyOffset ) ) ;
  if ( titleIndex == NULL )
...
{ 
  ...
  attrIndex = (struct attrKeyOffset *) calloc ( attrCount * 2, sizeof ( struct attrKeyOffset ) ) ;
  if ( attrIndex == NULL )
...
{ 
  ...
  namesIndex = (struct nameKeyOffset *) calloc ( nameCount * 2, sizeof ( struct nameKeyOffset ) ) ;
  if ( namesIndex == NULL )
...
void makeCastDatabaseNamesIndex ( int listId, NameID namesOnList )
{
  ...
  namesIndex = (struct nameKeyOffset *) calloc ( namesOnList * 2, sizeof ( struct nameKeyOffset ) ) ;
  if ( namesIndex == NULL )
    moviedbError ( "mkdb: not enough memory to generate names index" ) ;
...
void makeCastDatabaseTitlesIndex ( int listId, long nentries )
{
  ...
  titlesIndex = (struct titleKeyOffset *) calloc ( nentries * 2, sizeof ( struct titleKeyOffset ) ) ;
  if ( titlesIndex == NULL )
    moviedbError ( "mkdb: not enough memory to generate titles index" ) ;
...
int main (int argc, char **argv)
{
  ...
  for ( i = 1; i < argc; i++ )
    ...
    else
    {
      ...
      if ( ! found )
      {
        (void) fprintf ( stdout, "mkdb: unrecognised option %s\n", argv[i] ) ;
        err = TRUE ;
      }
    }
