...
int main ( int argc, char **argv )
{
  ...
  for ( i = 1; i < argc; i++ )
    ...
    {
      (void) fprintf ( stdout, "lguide: unrecognised option %s\n", argv[i] ) ;
      err = TRUE ;
    }

  if ( fname [ 0 ] == '\0' )
  {
    (void) fprintf ( stdout, "lguide: no input file specified\n" ) ;
    err = TRUE ;
  }
  else
    if ( !err )
  ...
  if ( !err && tchain == NULL )
  {
    (void) fprintf ( stdout, "lguide: no titles found\n" ) ;
    err = TRUE ;
  }
