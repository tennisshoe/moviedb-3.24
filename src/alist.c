...
int main ( int argc, char **argv )
{
  ...
  for ( i = 1; i < argc; i++ )
	...
	if ( strcmp ( filmographyDefs [ j ] . option, argv [ i ] ) == 0 )
      ...
      if ( ! okopt )
      {
         (void) fprintf ( stdout, "alist: unrecognised option %s\n", argv[i] ) ;
         err = TRUE ;
      }
    }
  ...
  {
    (void) fprintf ( stdout, "alist: missing attribute to search for\n" ) ;
    err = TRUE ;
  }
  
  if ( !err && titleFlag == FALSE && listId < -1 )
  {
    (void) fprintf ( stdout, "alist: must pick -title or one database option\n" ) ;
    err = TRUE ;
  }

  if ( !err && titleFlag && listId >= 0 )
  {
    (void) fprintf ( stdout, "alist: can't pick -title and a database option\n" ) ;
    err = TRUE ;
  }
