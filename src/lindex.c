...
int main ( int argc, char **argv )
{
  ...
  for ( i = 1; i < argc; i++ )
	...
	if ( strcmp ( filmographyDefs [ j ] . option, argv [ i ] ) == 0 )
      ...
      if ( ! okopt )
      {
         (void) fprintf ( stdout, "lindex: unrecognised option %s\n", argv[i] ) ;
         err = TRUE ;
      }
    }
  ...
  if ( nmopt && ttlopt )
  {
    (void) fprintf ( stdout, "lindex: can't mix title and name indexing in same command\n" ) ;
    err = TRUE ;
  }

  if ( !err && !nmopt && !ttlopt )
  {
    (void) fprintf ( stdout, "lindex: must specify at least one title or name indexing option\n" ) ;
    err = TRUE ;
  }
